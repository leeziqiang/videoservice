...
let dsSelect_take={
    ...
    "initEvent":function(obj){
        ...
        dsSelect_tools.evenAddBarData(obj);
    },
    /**
     * 设置左侧数据
     */
...
let dsSelect_tools = {
    ...
    //添加条形码头部数据事件
    "evenAddBarData":function (obj) {
        $('.dsButton').click(function(){
            $(".add-tr").remove();//删除原始表格元素
            let data = obj.getSelectRightValus();
            if (!arrayUtil.arrayIsEmpty(data)) {
                data.forEach(function (value, index, attr) {
                    var html = "<tr class='add-tr'><td>" + value.name + "</td><td>************</td></tr>";
                    $("#bar-code-foot").before(html);
                });
            }
        });
    },
